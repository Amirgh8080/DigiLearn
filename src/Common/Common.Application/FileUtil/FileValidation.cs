...
public static class FileValidation
{
    ...
    public static bool IsValidCompressedFile(this IFormFile file)
    {
        if (file == null) return false;
        var path = Path.GetExtension(file.FileName);
        path = path.ToLower();
    ...
    public static bool IsValidVideoFile(this IFormFile file)
    {
        ...
        var path = Path.GetExtension(file.FileName);
        path = path.ToLower();
        if (path == ".mp4" || path == ".mp3" || path == ".wav" ||
            path == ".m4a" || path == ".wmv" || path == ".mkv")
        {
            return true;
        }
        return false;
    ...
    public static bool IsValidImageFile(string fileName)
    {
        ...
        path = path.ToLower();
        if (path == ".jpg" || path == ".png" || path == ".bmp" || path == ".svg" ||
            path == ".jpeg" ||path ==".webp" )
        {
            return true;
        }
